...
function storyDrainHeal(attacker, move, dmg, log) {
    if (!attacker || !move || !(dmg > 0)) return;
    const name = (move.name || '').toLowerCase();
    if (!/drain|absorb|leech|draining|syphon|parabolic|oblivion|bitter blade|dream eater/.test(name)) return;
    let ratio = 0.5;
    if (/draining kiss|oblivion wing|dream eater/.test(name)) ratio = 0.75;
    else if (/leech life|horn leech|drain punch|bitter blade/.test(name)) ratio = 1;
    const before = attacker.hp;
    attacker.hp = Math.min(attacker.maxHp, attacker.hp + Math.max(1, Math.floor(dmg * ratio)));
    const gained = attacker.hp - before;
    if (gained > 0 && typeof capitalize === 'function' && Array.isArray(log)) {
        log.push(capitalize(attacker.pokemon.name) + ' assorbe ' + gained + ' HP!');
    }
}
...
window.renderStoryTeam = function() {
    ...
    story.team.forEach((m, i) => {
        ...
        // --- Selezione del lead: tocca un Pokémon vivo per schierarlo come primo in lotta ---
        const isActive = (i === story.activeSlot);
        const ko = m.hp <= 0;
        const tag = document.createElement('div');
        tag.style.cssText = 'font-family:var(--pixel);font-size:6px;margin-top:6px;text-align:center;color:'
            + (isActive ? '#66bb6a' : (ko ? 'var(--gray)' : 'var(--gold)')) + ';';
        tag.textContent = isActive ? '★ In campo' : (ko ? 'Esausto' : '▶ Schiera');
...
function showStoryEvoOverlay(member, evoData, callback) {
    ...
    // L'evoluzione NON azzera il moveset: si mantengono le mosse (e i PP) scelti dal giocatore.
    // Fallback solo nel caso anomalo in cui il membro non avesse alcuna mossa.
...
(function () {
    ...
    function buildActionArea(battleEl) {
        ...
        // Toggle dei menu: ripremere lo stesso pulsante chiude il menu invece di impilarne un altro,
        // e aprire un menu diverso rimuove sempre quello eventualmente già aperto.
        const toggleMenu = (kind, openFn) => {
            const alreadyOpen = battleEl.querySelector('.sb-menu') && battleEl.dataset.openMenu === kind;
            battleEl.querySelectorAll('.sb-menu').forEach(m => m.remove());
            if (alreadyOpen) { battleEl.dataset.openMenu = ''; return; }
            openFn();
            battleEl.dataset.openMenu = kind;
        };
        if (story._canCatch) addSub('\ud83c\udfaf Cattura', () => toggleMenu('catch', () => renderStoryCatchMenu(battleEl)));
        if (story.team.length > 1) addSub('\ud83d\udd04 Cambia', () => toggleMenu('switch', () => renderStorySwitchMenu(battleEl)));
        addSub('\ud83c\udf92 Borsa', () => toggleMenu('bag', () => renderStoryBagMenu(battleEl)));
...
// ===== PATCH: BOTTONE LIBERA POKÉMON IN SQUADRA =====
// Sovrascrive renderStoryTeam per aggiungere il bottone 🕊️ Libera su ogni card.
// Disabilitato se la squadra ha un solo membro.
(function() {
    const _origRenderStoryTeam = renderStoryTeam;
    window.renderStoryTeam = function() {
        _origRenderStoryTeam();
        // Dopo il render originale, aggiungi i bottoni Libera a ogni card
        const cards = document.querySelectorAll('.story-team-card');
        cards.forEach((card, idx) => {
            const btn = document.createElement('button');
            btn.className = 'story-util-btn';
            btn.textContent = '🕊️ Libera';
            btn.style.cssText = 'margin-top:8px;width:100%;font-size:11px;border-color:#ef5350;color:#ef5350;opacity:' + (story.team.length <= 1 ? '0.4' : '0.85') + ';';
            btn.disabled = story.team.length <= 1;
            btn.addEventListener('click', () => {
                if (story.team.length <= 1) return;
                const name = capitalize(story.team[idx].pokemon.name);
                story.team.splice(idx, 1);
                if (story.activeSlot >= story.team.length)
                    story.activeSlot = Math.max(0, story.team.length - 1);
                saveStory();
                renderStoryTeam();
            });
            card.appendChild(btn);
